...
const OneTable = ({_class, link, data, hr, lh, title}) => {

  let _handleOnMouse = (itemId, e) => hr(itemId, e)

  let _handleClick
  if(link) {
    _handleClick = (itemId, url, e) => lh(itemId, url, e)
  } else {
    _handleClick = () => {}
  }

  let materials = data.map(val => {
    return <li className="pea opacity05" onClick={_handleClick.bind(this, val.item_id, val.url)} data-parent={val.item_id} data-childs={'-' + val.rel + '-'} key={val.item_id} onMouseOver={_handleOnMouse.bind(this, val.item_id)}>
