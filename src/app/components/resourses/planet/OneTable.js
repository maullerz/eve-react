...
const OneTable = ({_class, data, hr, lh, title}) => {
  ...
  let materials = data.map(val => {
    return <li className="pea" onClick={_handleClick.bind(this, val.item_id, val.url)} data-parent={val.item_id} data-childs={'-' + val.rel + '-'} key={val.item_id} onMouseOver={_handleOnMouse.bind(this, val.item_id)}>
