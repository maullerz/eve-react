...
export function getStructures() {
  return dispatch => {
    return ApiService.Main.structures().then(json => {
      dispatch({
        type: MANUFACTURE_GET_STRUCTURES,
...
export function updateVariable(variable, value) {
  let payload = {
    type: MANUFACTURE_CHANGE_VARIABLE,
...
export function setComponentsSystem(systemId, props) {
  ...
  if (components.length) {
    ...
    cpms = cpms.concat(uniq(c));
  }
  return dispatch => {
    return ApiService.Main.prices(systemId, cpms).then(json => {
...
export function setItemSystem(systemId, props) {
  return dispatch => {
    return ApiService.Main.prices(systemId, props.item.item_id).then(json => {
...
export function searchBpc(term) {
  return dispatch => {
    return ApiService.Manufacture.searchBpc(term).then(res => {
...
export function getBpc(url) {
  return dispatch => {
    return ApiService.Manufacture.getBpc(url).then(res => {
